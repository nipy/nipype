...
class Level1Design(BaseInterface):
    ...
    def _post_run(self):
        ...
        for runno, runinfo in enumerate(self._format_session_info(self.inputs.session_info)):
            self.outputs.fsf_files.append(os.path.join(cwd, 'run%d.fsf' % runno))
            self.outputs.ev_files.insert(runno, [])
            evname = []
            for field in ['cond', 'regress']:
                for i, cond in enumerate(runinfo[field]):
                    name = cond['name']
                    ...
                    if field == 'cond':
                        if usetd:
                            evname.append(name + 'TD')
                    self.outputs.ev_files[runno].append(
                ...
                if line.find("set fmri(outputdir)") > -1:
                    try:
                        ...
                        if os.path.exists(outputdir_spec):
                            self.outputs.feat_dir = outputdir_spec

                    except:
                        pass
        if not self.outputs.feat_dir:
            if is_ica:
                self.outputs.feat_dir = glob(os.path.join(os.getcwd(), '*ica'))[0]
            else:
                self.outputs.feat_dir = glob(os.path.join(os.getcwd(), '*feat'))[0]
        IFLOGGER.info('Outputs from FEATmodel: %s', self.outputs)
...
class FEATModelInputSpec(FSLCommandInputSpec):
    ...
    def _format_arg(self, name, trait_spec, value):
        if name == 'fsf_file':
            return super(FEATModelInputSpec, self)._format_arg(
                name, trait_spec, self._get_design_root(value))
        elif name == 'ev_files':
            return ''
        else:
            return super(FEATModelInputSpec, self)._format_arg(name, trait_spec, value)


class FEATModelOutputSpec(TraitedSpec):
    design_file = File(
...
threshold=10, results_dir='stats')
    ...
    def _post_run(self):
        ...
        if pe_files:
            self.outputs.param_estimates = pe_files
        self.outputs.residual4d = self._gen_fname('res4d.nii', cwd=results_dir)
        self.outputs.dof_file = os.path.join(results_dir, 'dof')
        self.outputs.sigmasquareds = self._gen_fname('sigmasquareds.nii',
                                                   cwd=results_dir)
        self.outputs.thresholdac = self._gen_fname('threshac1.nii',
                                                 cwd=results_dir)
        if Info.version() and LooseVersion(Info.version()) < LooseVersion('5.0.7'):
            self.outputs.corrections = self._gen_fname('corrections.nii',
                                                     cwd=results_dir)
        self.outputs.logfile = self._gen_fname('logfile',
                                             change_ext=False,
                                             cwd=results_dir)
        ...
        if Info.version() and LooseVersion(Info.version()) > LooseVersion('5.0.6'):
            ...
            for i in range(numtcons):
                ...
                tstats.append(self._gen_fname('tstat%d.nii' % (base_contrast + i),
                                              cwd=pth))
            if copes:
                self.outputs.copes = copes
                self.outputs.varcopes = varcopes
                self.outputs.zstats = zstats
                self.outputs.tstats = tstats
            fstats = []
            zfstats = []
...
class FEATRegister(BaseInterface):
    ...
    def _post_run(self):
        self.outputs.fsf_file = os.path.abspath(
...
class FLAMEO(FSLCommand):
    ...
    def _post_run(self):
        ...
        self.outputs.var_copes = var_copes
...
class MultipleRegressDesign(BaseInterface):
    ...
    def _run_interface(self, runtime):
        ...
        # write design files
        with open(os.path.join(cwd, filename), 'wt') as out_file:
    ...
    def _post_run(self):
        nfcons = sum([1 for con in self.inputs.contrasts if con[1] == 'F'])
        for field, _ in list(self.outputs.items()):
...
class MELODIC(FSLCommand):
    ...
    def _post_run(self):
        ...
        if isdefined(self.inputs.report) and self.inputs.report:
            self.outputs.report_dir = os.path.join(
...
class Randomise(FSLCommand):
    ...
    def _post_run(self):
        ...
        if self.inputs.tfce or self.inputs.tfce2D:
            ...
            prefix = 'clusterm'
        if prefix:
            self.outputs.t_p_files = glob(self._gen_fname(
                                        '%s_%s_p_tstat*' % (self.inputs.base_name, prefix)))
            self.outputs.t_corrected_p_files = glob(self._gen_fname(
                '%s_%s_corrp_tstat*.nii' % (self.inputs.base_name, prefix)))

            self.outputs.f_p_files = glob(self._gen_fname(
                '%s_%s_p_fstat*.nii' % (self.inputs.base_name, prefix)))
            self.outputs.f_corrected_p_files = glob(self._gen_fname(
...
class GLM(FSLCommand):
    ...
    def _post_run(self):
        ...
        if isdefined(self.inputs.out_cope):
            self.outputs.out_cope = os.path.abspath(self.inputs.out_cope)

        if isdefined(self.inputs.out_z_name):
            self.outputs.out_z = os.path.abspath(self.inputs.out_z_name)

        if isdefined(self.inputs.out_t_name):
            self.outputs.out_t = os.path.abspath(self.inputs.out_t_name)

        if isdefined(self.inputs.out_p_name):
            self.outputs.out_p = os.path.abspath(self.inputs.out_p_name)

        if isdefined(self.inputs.out_f_name):
            self.outputs.out_f = os.path.abspath(self.inputs.out_f_name)

        if isdefined(self.inputs.out_pf_name):
            self.outputs.out_pf = os.path.abspath(self.inputs.out_pf_name)

        if isdefined(self.inputs.out_res_name):
            self.outputs.out_res = os.path.abspath(self.inputs.out_res_name)

        if isdefined(self.inputs.out_varcb_name):
            self.outputs.out_varcb = os.path.abspath(self.inputs.out_varcb_name)

        if isdefined(self.inputs.out_sigsq_name):
            self.outputs.out_sigsq = os.path.abspath(self.inputs.out_sigsq_name)

        if isdefined(self.inputs.out_data_name):
            self.outputs.out_data = os.path.abspath(self.inputs.out_data_name)

        if isdefined(self.inputs.out_vnscales_name):
            self.outputs.out_vnscales = os.path.abspath(
