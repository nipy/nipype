...
class FILMGLSInputSpec505(FSLCommandInputSpec):
    ...
    multitaper_product = traits.Int(argstr='--mt=%d', xor=_estimate_xor,
                                    desc=('multitapering with slepian tapers '
                                          'and num is the time-bandwidth '
                                          'product'))
    use_pava = traits.Bool(argstr='--pava',
...
threshold=10, results_dir='stats')
    ...
    def _list_outputs(self):
        ...
        outputs['thresholdac'] = self._gen_fname('threshac1.nii',
                                                 cwd=results_dir)
        if (Info.version() and
                LooseVersion(Info.version()) < LooseVersion('5.0.7')):
        ...
        if (Info.version() and
                LooseVersion(Info.version()) > LooseVersion('5.0.6')):
            pth = results_dir
            ...
            for i in range(numtcons):
                ...
                varcopes.append(
                    self._gen_fname('varcope%d.nii' % (base_contrast + i),
                                    cwd=pth))
                zstats.append(self._gen_fname(
                    'zstat%d.nii' % (base_contrast + i), cwd=pth))
                tstats.append(self._gen_fname(
...
class ContrastMgrInputSpec(FSLCommandInputSpec):
    ...
    param_estimates = InputMultiPath(File(exists=True),
                                     ...
                                     desc=('Parameter estimates for each '
                                           'column of the design matrix'))
    corrections = File(
        exists=True, copyfile=False, mandatory=True,
...
class SMM(FSLCommand):
    ...
    def _list_outputs(self):
        ...
        if (not isdefined(self.inputs.no_deactivation_class) or not
                self.inputs.no_deactivation_class):
...
class ClusterInputSpec(FSLCommandInputSpec):
    ...
    fractional = traits.Bool(
        False, usedefault=True, argstr='--fractional',
                ...
                if isinstance(inval, bool):
                    if inval:
                        ...
                        outputs[outkey] = self._gen_fname(
                            self.inputs.in_file, suffix='_' + suffix,
...
class GLMInputSpec(FSLCommandInputSpec):
    ...
    contrasts = File(exists=True, argstr='-c %s',
