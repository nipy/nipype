...
def create_files_in_directory():
    ...
    out_ext = Info.ftypes[Info.output_type()]
...
@skipif(no_fsl)
def test_maths_base(fsl_output_type='NIFTI'):
    files, testdir, origdir, out_ext = create_files_in_directory()

    # Get some fslmaths
    maths = fsl.MathsCommand(output_type=fsl_output_type)
    ...
    for dtype in dtypes:
        foo = fsl.MathsCommand(in_file="a.nii", internal_datatype=dtype, output_type=fsl_output_type)
        yield assert_equal, foo.cmdline, int_cmdline % dtype
        bar = fsl.MathsCommand(in_file="a.nii", output_datatype=dtype, output_type=fsl_output_type)
        yield assert_equal, bar.cmdline, out_cmdline % dtype
        foobar = fsl.MathsCommand(in_file="a.nii", internal_datatype=dtype, output_datatype=dtype, output_type=fsl_output_type)
...
@skipif(no_fsl)
def test_changedt(fsl_output_type='NIFTI'):
    files, testdir, origdir, out_ext = create_files_in_directory()

    # Get some fslmaths
    cdt = fsl.ChangeDataType(output_type=fsl_output_type)
    ...
    for dtype in dtypes:
        foo = fsl.MathsCommand(in_file="a.nii", out_file="b.nii", output_datatype=dtype, output_type=fsl_output_type)
...
@skipif(no_fsl)
def test_threshold(fsl_output_type='NIFTI'):
    files, testdir, origdir, out_ext = create_files_in_directory()

    # Get the command
    thresh = fsl.Threshold(in_file="a.nii", out_file="b.nii", output_type=fsl_output_type)
    ...
    val = "%.10f" % 42
    thresh = fsl.Threshold(in_file="a.nii", out_file="b.nii", thresh=42, use_robust_range=True, output_type=fsl_output_type)
    yield assert_equal, thresh.cmdline, cmdline % ("-thrp " + val)
    thresh.inputs.use_nonzero_voxels = True
    yield assert_equal, thresh.cmdline, cmdline % ("-thrP " + val)
    thresh = fsl.Threshold(in_file="a.nii", out_file="b.nii", thresh=42, direction="above", output_type=fsl_output_type)
...
@skipif(no_fsl)
def test_meanimage(fsl_output_type='NIFTI'):
    files, testdir, origdir, out_ext = create_files_in_directory()

    # Get the command
    meaner = fsl.MeanImage(in_file="a.nii", out_file="b.nii", output_type=fsl_output_type)
    ...
    meaner = fsl.MeanImage(in_file="a.nii", output_type=fsl_output_type)
...
@skipif(no_fsl)
def test_maximage(fsl_output_type='NIFTI'):
    files, testdir, origdir, out_ext = create_files_in_directory()

    # Get the command
    maxer = fsl.MaxImage(in_file="a.nii", out_file="b.nii", output_type=fsl_output_type)
    ...
    # Test the auto naming
    maxer = fsl.MaxImage(in_file="a.nii", output_type=fsl_output_type)
...
@skipif(no_fsl)
def test_smooth(fsl_output_type='NIFTI'):
    files, testdir, origdir, out_ext = create_files_in_directory()

    # Get the command
    smoother = fsl.IsotropicSmooth(in_file="a.nii", out_file="b.nii", output_type=fsl_output_type)
    ...
    smoother = fsl.IsotropicSmooth(in_file="a.nii", sigma=5, output_type=fsl_output_type)
...
@skipif(no_fsl)
def test_mask(fsl_output_type='NIFTI'):
    files, testdir, origdir, out_ext = create_files_in_directory()

    # Get the command
    masker = fsl.ApplyMask(in_file="a.nii", out_file="c.nii", output_type=fsl_output_type)
    ...
    masker = fsl.ApplyMask(in_file="a.nii", mask_file="b.nii", output_type=fsl_output_type)
...
def test_dilation(fsl_output_type=None):
    ...
    # Get the command
    diller = fsl.DilateImage(in_file="a.nii", out_file="b.nii", output_type=fsl_output_type)
    ...
    dil = fsl.DilateImage(in_file="a.nii", operation="max", output_type=fsl_output_type)
...
def test_erosion(fsl_output_type=None):
    ...
    # Get the command
    erode = fsl.ErodeImage(in_file="a.nii", out_file="b.nii", output_type=fsl_output_type)
    ...
    # Test that we don't need to request an out name
    erode = fsl.ErodeImage(in_file="a.nii", output_type=fsl_output_type)
...
def test_spatial_filter(fsl_output_type=None):
    ...
    # Get the command
    spfilt = fsl.SpatialFilter(in_file="a.nii", out_file="b.nii", output_type=fsl_output_type)

    # Test the underlying command
    yield assert_equal, spfilt.cmd, "fslmaths"

    # Test that it fails without an operation
    yield assert_raises, ValueError, spfilt.run

    # Test the different operations
    for op in ["mean", "meanu", "median"]:
        spfilt.inputs.operation = op
        yield assert_equal, spfilt.cmdline, "fslmaths a.nii -f%s b.nii" % op

    # Test that we don't need to ask for an out name
    spfilt = fsl.SpatialFilter(in_file="a.nii", operation="mean", output_type=fsl_output_type)
    yield assert_equal, spfilt.cmdline, "fslmaths a.nii -fmean %s" % os.path.join(testdir, "a_filt%s" % out_ext)
...
def test_unarymaths(fsl_output_type=None):
    ...
    # Get the command
    maths = fsl.UnaryMaths(in_file="a.nii", out_file="b.nii", output_type=fsl_output_type)
    ...
    for op in ops:
        maths = fsl.UnaryMaths(in_file="a.nii", operation=op, output_type=fsl_output_type)
...
def test_binarymaths(fsl_output_type=None):
    ...
    # Get the command
    maths = fsl.BinaryMaths(in_file="a.nii", out_file="c.nii", output_type=fsl_output_type)
    ...
    for op in ops:
        for ent in operands:
            maths = fsl.BinaryMaths(in_file="a.nii", out_file="c.nii", operation=op, output_type=fsl_output_type)
            if ent == "b.nii":
    ...
    # Test that we don't need to ask for an out file
    for op in ops:
        maths = fsl.BinaryMaths(in_file="a.nii", operation=op, operand_file="b.nii", output_type=fsl_output_type)
...
def test_multimaths(fsl_output_type=None):
    ...
    # Get the command
    maths = fsl.MultiImageMaths(in_file="a.nii", out_file="c.nii", output_type=fsl_output_type)
    ...
    # Test that we don't need to ask for an out file
    maths = fsl.MultiImageMaths(in_file="a.nii", op_string="-add %s -mul 5", operand_files=["b.nii"], output_type=fsl_output_type)
...
def test_tempfilt(fsl_output_type=None):
    ...
    # Get the command
    filt = fsl.TemporalFilter(in_file="a.nii", out_file="b.nii", output_type=fsl_output_type)
    ...
    # Test that we don't need to ask for an out file
    filt = fsl.TemporalFilter(in_file="a.nii", highpass_sigma=64, output_type=fsl_output_type)
