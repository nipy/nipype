...
def crawl_code_struct(code_struct, package_dir):
    ...
    for k, v in code_struct.items():
        if isinstance(v, str) or isinstance(v, (str, bytes)):
            ...
            class_name = k
            class_code = v
            add_class_to_package([class_code], [class_name], module_name,
                                 package_dir)
        else:
            l1 = {}
            l2 = {}
...
""".format(pkg_name=package_dir.split("/")[-1],
           sub_pks="\n    ".join([
               "config.add_data_dir('%s')" % sub_pkg for sub_pkg in subpackages
           ])))
            f.close()


def generate_all_classes(modules_list=[],
                         launcher=[],
                         redirect_x=False,
                         mipav_hacks=False):
...
def generate_class(module,
    ...
    for desc_str in [
            'title', 'category', 'description', 'version', 'documentation-url',
    ...
    for paramGroup in dom.getElementsByTagName("parameters"):
        ...
        for param in paramGroup.childNodes:
            ...
            if param.getElementsByTagName(
                ...
                traitsParams["desc"] = param.getElementsByTagName(
                    'description')[0].firstChild.nodeValue.replace(
                        '"', "\\\"").replace("\n", ", ")
            ...
            if param.nodeName.endswith('-enumeration'):
                ...
                if param.nodeName in [
                    ...
                    values = [
                        "%s(exists=True)" % typesDict[param.nodeName.replace(
                            '-vector', '')]
                    ]
                else:
            ...
            if param.nodeName in [
                ...
                if not param.getElementsByTagName('channel'):
                    raise RuntimeError(
                        ...
                        format(traitsParams))
                elif param.getElementsByTagName('channel')[
                        0].firstChild.nodeValue == 'output':
                    ...
                    outputTraits.append("%s = %s(%s%s)" %
                                        (name, type.replace("Input", "Output"),
                                         parse_values(values),
                                         parse_params(traitsParams)))

                    outputs_filenames[name] = gen_filename_from_param(
                        param, name)
                elif param.getElementsByTagName('channel')[
                        0].firstChild.nodeValue == 'input':
                    if param.nodeName in [
    ...
    if mipav_hacks:
        ...
        inputTraits = [
            trait for trait in inputTraits
            if trait.split()[0] not in blacklisted_inputs
        ]
    ...
    main_class = template.replace('%class_str%', class_string).replace(
        "%module_name%", module_name).replace("%name%", module).replace(
            "%output_filenames_code%", output_filenames_code).replace(
...
def grab_xml(module, launcher, mipav_hacks=False):
    ...
    if mipav_hacks:
        ...
        if xmlReturnValue.strip().startswith(
                "Error: Unable to set default atlas"):
            xmlReturnValue = xmlReturnValue.strip()[len(
                "Error: Unable to set default atlas"):]
    try:
