...
class bseInputSpec(CommandLineInputSpec):
    ...
    inputMRIFile = File(exists=True, mandatory=True, argstr='-i %s', desc='input MRI volume', position=0)

    outputMRIVolume = File(mandatory=False, desc='output brain-masked MRI volume. If unspecified, output file name will be auto generated.', argstr='-o %s', position=1, hash_files=False, genfile=True)

    diffusionConstant = traits.Float(25, usedefault=True, desc='diffusion constant [default if unspecified: 25]' , argstr='-d %f')

    diffusionIterations = traits.Int(3, usedefault=True, desc='diffusion iterations [default if unspecified: 3]', argstr='-n %d')

    edgeDetectionConstant = traits.Float(0.64, usedefault=True, desc='edge detection constant [default if unspecified: 0.64]', argstr='-s %f')

    radius = traits.Float(1, usedefault=True, desc='radius of erosion/dilation filter [default if unspecified: 1]', argstr='-r %f')

    dilateFinalMask = traits.Bool(True, usedefault=True, desc='dilate final mask [default if unspecified: True]', argstr='-p')

    trim = traits.Bool(True, usedefault=True, desc='trim brainstem [default if unspecified: True]', argstr='--trim')

    outputMaskFile = File(mandatory=False, desc='save smooth brain mask' , argstr='--mask %s', hash_files=False)
    ...
    verbosityLevel = traits.Float(1, usedefault=True, desc=' verbosity level (0=silent) [default if unspecified: 1]', argstr='-v %f')

    noRotate = traits.Bool(True, usedefault=True,  desc='retain original orientation (default behavior will auto-rotate input NII files to LPI orientation) [default if unspecified: True]', argstr='--norotate')
...
class bse(CommandLine):
    ...
    def _list_outputs(self):
        ...
        if isdefined(self.inputs.outputMRIVolume):
            outputs['outputMRIVolume'] = (os.getcwd() + '/' + self.inputs.outputMRIVolume)
        else:
            outputs['outputMRIVolume'] = self._gen_filename('outputMRIVolume')
            
            
        if isdefined(self.inputs.outputMaskFile):
            outputs['outputMaskFile'] = (os.getcwd() + '/' + self.inputs.outputMaskFile)
        if isdefined(self.inputs.outputDiffusionFilter):
            outputs['outputDiffusionFilter'] = (os.getcwd() + '/' + self.inputs.outputDiffusionFilter)
        if isdefined(self.inputs.outputEdgeMap):
            outputs['outputEdgeMap'] = (os.getcwd() + '/' + self.inputs.outputEdgeMap)
        if isdefined(self.inputs.outputDetailedBrainMask):
            outputs['outputDetailedBrainMask'] = (os.getcwd() + '/' + self.inputs.outputDetailedBrainMask)
        if isdefined(self.inputs.outputCortexFile):
            outputs['outputCortexFile'] = (os.getcwd() + '/' + self.inputs.outputCortexFile)   
...
class bfcInputSpec(CommandLineInputSpec):
    inputMRIFile = File(exists = True, mandatory=True, desc='input skull-stripped MRI volume', argstr='-i %s', position=0)

    outputMRIVolume = File(mandatory=False, desc='output bias-corrected MRI volume. If unspecified, output file name will be auto generated.', argstr='-o %s', position=1, hash_files=False, genfile=True)
    ...
    minBias = traits.Float(0.5, usedefault=True, desc='minimum allowed bias value [default if unspecified: 0.5]', argstr='-L %f')

    maxBias = traits.Float(1.5, usedefault=True, desc='maximum allowed bias value [default if unspecified: 1.5]', argstr='-U %f')

    biasRange = traits.Enum("low", "medium", "high", desc="Preset options for bias_model\nlow: small bias model [0.95,1.05]\nmedium: medium bias model [0.90,1.10]\nhigh: high bias model [0.80,1.20]", argstr='%s')
...
class bfc(CommandLine):
    ...
    def _list_outputs(self):
        ...
        if isdefined(self.inputs.outputBiasField):
                outputs['outputBiasField'] = (os.getcwd() + '/' + self.inputs.outputBiasField)
        if isdefined(self.inputs.outputMaskedBiasField):
                outputs['outputMaskedBiasField'] = (os.getcwd() + '/' + self.inputs.outputMaskedBiasField)
        if isdefined(self.inputs.correctionScheduleFile):
                outputs['correctionScheduleFile'] = (os.getcwd() + '/' + self.inputs.correctionScheduleFile)
...
class pvcInputSpec(CommandLineInputSpec):
    ...
    outputLabelFile = File(mandatory=False, desc='output label file. If unspecified, output file name will be auto generated.', argstr='-o %s', genfile=True)
    outputTissueFractionFile = File(mandatory=False, desc='output tissue fraction file', argstr='-f %s', genfile=True)
...
class pvc(CommandLine):
    ...
    def _list_outputs(self):
        ...
        if isdefined(self.inputs.outputLabelFile):
            outputs['outputLabelFile'] = (os.getcwd() + '/' + self.inputs.outputLabelFile)
        else:
        ...
        if isdefined(self.inputs.outputTissueFractionFile):
            ...
            outputs['outputTissueFractionFile'] = self._gen_filename('outputTissueFractionFile')
...
class cerebroInputSpec(CommandLineInputSpec):
    ...
    outputCerebrumMaskFile = File(mandatory=False, desc='output cerebrum mask volume. If unspecified, output file name will be auto generated.', argstr='-o %s', genfile=True)
    outputLabelMaskFile = File(mandatory=False, desc='output labeled hemisphere/cerebrum volume. If unspecified, output file name will be auto generated.', argstr='-l %s', genfile=True)
    costFunction = traits.Int(2, usedefault=True, desc='0,1,2 [default if unspecified: 2]', argstr='-c %d')
    useCentroids = traits.Bool(desc='use centroids of data to initialize position', argstr='--centroids')
    outputAffineTransformFile = File(desc='save affine transform to file.', argstr='--air %s')
    outputWarpTransformFile = File(desc='save warp transform to file.', argstr='--warp %s')
    verbosity = traits.Int(desc='verbosity level (0=silent)', argstr='-v %d')
...
class cerebro(CommandLine):
    ...
    def _list_outputs(self):
        ...
        if isdefined(self.inputs.outputCerebrumMaskFile):
            outputs['outputCerebrumMaskFile'] = (os.getcwd() + '/' + self.inputs.outputCerebrumMaskFile)
        else:
            outputs['outputCerebrumMaskFile'] = self._gen_filename('outputCerebrumMaskFile')


        if isdefined(self.inputs.outputLabelMaskFile):
            outputs['outputLabelMaskFile'] = (os.getcwd() + '/' + self.inputs.outputLabelMaskFile)
        else:
            outputs['outputLabelMaskFile'] = self._gen_filename('outputLabelMaskFile')


        if isdefined(self.inputs.outputAffineTransformFile):
            outputs['outputAffineTransformFile'] = (os.getcwd() + '/' + self.inputs.outputAffineTransformFile)
        if isdefined(self.inputs.outputWarpTransformFile):
            outputs['outputWarpTransformFile'] = (os.getcwd() + '/' + self.inputs.outputWarpTransformFile)
...
class cortexInputSpec(CommandLineInputSpec):
    ...
    outputCerebrumMask = File(mandatory=False, desc='output structure mask. If unspecified, output file name will be auto generated.', argstr='-o %s', genfile=True)
    inputTissueFractionFile = File(mandatory=True, desc='tissue fraction file (32-bit float)', argstr='-f %s')
    tissueFractionThreshold = traits.Float(50.0, usedefault=True, desc='tissue fraction threshold (percentage) [default if unspecified: 50]', argstr='-p %f')
    computeWGBoundary = traits.Bool(True, usedefault=True, desc='compute WM/GM boundary [default if unspecified: True]', argstr='-w')
    computeGCBoundary = traits.Bool(desc='compute GM/CSF boundary', argstr='-g')
    includeAllSubcorticalAreas = traits.Bool(True, usedefault=True, esc='include all subcortical areas in WM mask [default if unspecified: True]', argstr='-a')
...
class cortex(CommandLine):
    ...
    def _list_outputs(self):
        ...
        if isdefined(self.inputs.outputCerebrumMask):
            ...
            outputs['outputCerebrumMask'] = self._gen_filename('outputCerebrumMask')
...
class scrubmaskInputSpec(CommandLineInputSpec):
    ...
    outputMaskFile = File(mandatory=False, desc='output structure mask file. If unspecified, output file name will be auto generated.', argstr='-o %s', genfile=True)
    backgroundFillThreshold = traits.Int(2, usedefault=True, desc='background fill threshold [default if unspecified: 2]', argstr='-b %d')
    foregroundTrimThreshold = traits.Int(0, usedefault=True, desc='foreground trim threshold [default if unspecified: 0]', argstr='-f %d')
...
class tcaInputSpec(CommandLineInputSpec):
    ...
    outputMaskFile = File(mandatory=False, desc='output mask volume. If unspecified, output file name will be auto generated.', argstr='-o %s', genfile=True)
    minCorrectionSize = traits.Int(2500, usedefault=True, desc='maximum correction size [default if unspecified: 2500]', argstr='-m %d')
    maxCorrectionSize = traits.Int(desc='minimum correction size', argstr='-n %d')
    foregroundDelta = traits.Int(20, usedefault=True, desc='foreground delta [default if unspecified: 20]', argstr='--delta %d')
...
class dewispInputSpec(CommandLineInputSpec):
    ...
    outputMaskFile = File(mandatory=False, desc='output file. If unspecified, output file name will be auto generated.', argstr='-o %s', genfile=True)
...
class dfsInputSpec(CommandLineInputSpec):
    ...
    outputSurfaceFile = File(mandatory=False, desc='output surface mesh file. If unspecified, output file name will be auto generated.', argstr='-o %s', genfile=True)
    inputShadingVolume = File(desc='shade surface model with data from image volume', argstr='-c %s')
    smoothingIterations = traits.Int(10, usedefault=True, desc='number of smoothing iterations [default if unspecified: 10]', argstr='-n %d')
    smoothingConstant = traits.Float(0.5, usedefault=True, desc='smoothing constant [default if unspecified: 0.5]', argstr='-a %f')
    curvatureWeighting = traits.Float(5.0, usedefault=True, desc='curvature weighting [default if unspecified: 5.0]', argstr='-w %f')
...
class dfs(CommandLine):
    ...
    def _list_outputs(self):
        ...
        if isdefined(self.inputs.outputSurfaceFile):
            outputs['outputSurfaceFile'] = (os.getcwd() + '/' + self.inputs.outputSurfaceFile)
        else:
            outputs['outputSurfaceFile'] = self._gen_filename('outputSurfaceFile')
...
class pialmeshInputSpec(CommandLineInputSpec):
    ...
    outputSurfaceFile = File(mandatory=False, desc='output file. If unspecified, output file name will be auto generated.', argstr='-o %s', genfile=True)
    verbosity = traits.Int(desc='verbosity', argstr='-v %d')
    inputTissueFractionFile = File(mandatory=True, desc='floating point (32) tissue fraction image', argstr='-f %s')
    numIterations = traits.Int(100, usedefault=True, desc='number of iterations [default if unspecified: 100]', argstr='-n %d')
    searchRadius = traits.Float(1, usedefault=True, desc='search radius [default if unspecified: 1]', argstr='-r %f')
    stepSize = traits.Float(0.4, usedefault=True, desc='step size [default if unspecified: 0.4]', argstr='-s %f')
    inputMaskFile = File(mandatory=True, desc='restrict growth to mask file region', argstr='-m %s')
    maxThickness = traits.Float(20, usedefault=True, desc='maximum allowed tissue thickness [default if unspecified: 20]', argstr='--max %f')
    tissueThreshold = traits.Float(1.05, usedefault=True, desc='tissue threshold [default if unspecified: 1.05]', argstr='-t %f')
# output interval is not an output -- it specifies how frequently the output surfaces are generated
    outputInterval = traits.Int(10, usedefault=True, desc='output interval [default if unspecified: 10]', argstr='--interval %d')
    exportPrefix = traits.Str(desc='prefix for exporting surfaces if interval is set', argstr='--prefix %s')
    laplacianSmoothing = traits.Float(0.025, usedefault=True, desc='apply Laplacian smoothing [default if unspecified: 0.025]', argstr='--smooth %f')
    timer = traits.Bool(desc='show timing', argstr='--timer')
    recomputNormals = traits.Bool(desc='recompute normals at each iteration', argstr='--norm')
    normalSmoother = traits.Float(0.2, usedefault=True, desc='strength of normal smoother. [default if unspecified: 0.2]', argstr='--nc %f')
...
class skullfinderInputSpec(CommandLineInputSpec):
    ...
    outputLabelFile = File(mandatory=False, desc='output file. If unspecified, output file name will be auto generated.', argstr='-o %s', genfile=True)
    verbosity = traits.Int(desc='verbosity', argstr='-v %d')
...
class hemisplitInputSpec(CommandLineInputSpec):
    ...
    outputLeftHemisphere = File(mandatory=False, desc='output surface file, left hemisphere. If unspecified, output file name will be auto generated.', argstr='--left %s', genfile=True)
    outputRightHemisphere = File(mandatory=False, desc='output surface file, right hemisphere. If unspecified, output file name will be auto generated.', argstr='--right %s', genfile=True)
    pialSurfaceFile = File(desc='pial surface file -- must have same geometry as input surface', argstr='-p %s')
    outputLeftPialHemisphere = File(mandatory=False, desc='output pial surface file, left hemisphere. If unspecified, output file name will be auto generated.', argstr='-pl %s', genfile=True)
    outputRightPialHemisphere = File(mandatory=False, desc='output pial surface file, right hemisphere. If unspecified, output file name will be auto generated.', argstr='-pr %s', genfile=True)
    verbosity = traits.Int(desc='verbosity (0 = silent)', argstr='-v %d')
    timer = traits.Bool(desc='timing function', argstr='--timer')
...
class hemisplit(CommandLine):
    ...
    def _list_outputs(self):
        ...
        if isdefined(self.inputs.outputLeftHemisphere):
                outputs['outputLeftHemisphere'] = (os.getcwd() + '/' + self.inputs.outputLeftHemisphere)
        else:
                outputs['outputLeftHemisphere'] = self._gen_filename('outputLeftHemisphere')

        if isdefined(self.inputs.outputRightHemisphere):
                outputs['outputRightHemisphere'] = (os.getcwd() + '/' + self.inputs.outputRightHemisphere)
        else:
                outputs['outputRightHemisphere'] = self._gen_filename('outputRightHemisphere')



        if isdefined(self.inputs.outputLeftPialHemisphere):
                outputs['outputLeftPialHemisphere'] = (os.getcwd() + '/' + self.inputs.outputLeftPialHemisphere)
        if isdefined(self.inputs.outputRightPialHemisphere):
                outputs['outputRightPialHemisphere'] = (os.getcwd() + '/' + self.inputs.outputRightPialHemisphere)

        return outputs


#removes directory of a pathway to a file, removes extension, returns file name up to first occurence of three consecutive underscore characters (if any)
#ex: /home/abc/testData___bseOutput_output_file.nii.gz --> testData
def getFileName(string):
    underscoreRegex = regex.compile("[^___]+")
    dotRegex = regex.compile("[^.]+")
    slashRegex = regex.compile("[^/]+")
    arr = underscoreRegex.findall(string)
