...
pvc_methods = [
    'GTM', 'IY', 'IY+RL', 'IY+VC', 'LABBE', 'LABBE+MTC', 'LABBE+MTC+RL',
    'LABBE+MTC+VC', 'LABBE+RBV', 'LABBE+RBV+RL', 'LABBE+RBV+VC', 'MG', 'MG+RL',
    'MG+VC', 'MTC', 'MTC+RL', 'MTC+VC', 'RBV', 'RBV+RL', 'RBV+VC', 'RL', 'VC'
]


class PETPVCInputSpec(CommandLineInputSpec):
    in_file = File(
        desc="PET image file", exists=True, mandatory=True, argstr="-i %s")
    out_file = File(
        desc="Output file", genfile=True, hash_files=False, argstr="-o %s")
    mask_file = File(
...
class PETPVC(CommandLine):
    ...
    references_ = [{
        ...
        BibTeX(
            ...
            "}"),
        'description':
        'PETPVC software implementation publication',
        'tags': ['implementation'],
    }]
