...
class ModelFitInputSpec(StdOutCommandLineInputSpec):
    ...
    outlier = File(
        argstr="-outliermap %s",
        exists=True,
        desc="Specifies the name of the file to contain the outlier map generated by the RESTORE algorithm.",
    )

    noisemap = File(
        argstr="-noisemap %s",
        exists=True,
        desc="Specifies the name of the file to contain the estimated noise variance on the diffusion-weighted signal, generated by a weighted tensor fit. The data type of this file is big-endian double.",
    )

    residualmap = File(
        argstr="-residualmap %s",
        exists=True,
        desc="Specifies the name of the file to contain the weighted residual errors after computing a weighted linear tensor fit. One value is produced per measurement, in voxel order.The data type of this file is big-endian double. Images of the residuals for each measurement can be extracted with shredder.",
    )

    sigma = traits.Float(
        argstr="-sigma %G",
        desc="Specifies the standard deviation of the noise in the data. Required by the RESTORE algorithm.",
    )

    bgthresh = traits.Float(
        argstr="-bgthresh %G",
        desc="Sets a threshold on the average q=0 measurement to separate foreground and background. The program does not process background voxels, but outputs the same number of values in background voxels and foreground voxels. Each value is zero in background voxels apart from the exit code which is -1.",
    )

    bgmask = File(
        argstr="-bgmask %s",
        exists=True,
        desc="Provides the name of a file containing a background mask computed using, for example, FSL's bet2 program. The mask file contains zero in background voxels and non-zero in foreground.",
    )

    cfthresh = traits.Float(
        argstr="-csfthresh %G",
        desc="Sets a threshold on the average q=0 measurement to determine which voxels are CSF. This program does not treat CSF voxels any different to other voxels.",
    )
    ...
    fixedbvalue = traits.List(
        ...
        minlen=3,
        maxlen=3,
        desc="As above, but specifies <M> <N> <b>. The resulting scheme is the same whether you specify b directly or indirectly using -fixedmodq.",
    )

    tau = traits.Float(
        argstr="-tau %G",
        desc="Sets the diffusion time separately. This overrides the diffusion time specified in a scheme file or by a scheme index for both the acquisition scheme and in the data synthesis.",
    )
...
class TrackBedpostxProbaInputSpec(TrackInputSpec):
    ...
    iterations = traits.Int(
        ...
        units="NA",
        desc=(
            "Number of streamlines to generate at each " "seed point. The default is 1."
        ),
    )
...
class TrackBayesDiracInputSpec(TrackInputSpec):
    ...
    pdf = traits.Enum(
        ...
        "acg",
        argstr="-pdf %s",
        desc='Specifies the model for PICo priors (not the curvature priors). The default is "bingham".',
    )

    pointset = traits.Int(
        argstr="-pointset %s",
        desc="Index to the point set to use for Bayesian likelihood calculation. The index specifies a set of evenly distributed points on the unit sphere, where each point x defines two possible step directions (x or -x) for the streamline path. A larger number indexes a larger point set, which gives higher angular resolution at the expense of computation time. The default is index 1, which gives 1922 points, index 0 gives 1082 points, index 2 gives 3002 points.",
    )
    ...
    datamodel = traits.Enum(
        ...
        "ballstick",
        argstr="-datamodel %s",
        desc='Model of the data for Bayesian tracking. The default model is "cylsymmdt", a diffusion tensor with cylindrical symmetry about e_1, ie L1 >= L_2 = L_3. The other model is "ballstick", the partial volume model (see ballstickfit).',
    )

    curvepriork = traits.Float(
        argstr="-curvepriork %G",
        desc="Concentration parameter for the prior distribution on fibre orientations given the fibre orientation at the previous step. Larger values of k make curvature less likely.",
    )

    curvepriorg = traits.Float(
        argstr="-curvepriorg %G",
        desc="Concentration parameter for the prior distribution on fibre orientations given the fibre orientation at the previous step. Larger values of g make curvature less likely.",
    )

    extpriorfile = File(
        exists=True,
        argstr="-extpriorfile %s",
        desc="Path to a PICo image produced by picopdfs. The PDF in each voxel is used as a prior for the fibre orientation in Bayesian tracking. The prior image must be in the same space as the diffusion data.",
    )
...
class TrackBootstrapInputSpec(TrackInputSpec):
    ...
    inversion = traits.Int(
        argstr="-inversion %s",
        desc="Tensor reconstruction algorithm for repetition bootstrapping. Default is 1 (linear reconstruction, single tensor).",
    )
    ...
    bsdatafiles = traits.List(
        ...
        mandatory=True,
        argstr="-bsdatafile %s",
        desc="Specifies files containing raw data for repetition bootstrapping. Use -inputfile for wild bootstrap data.",
    )
...
class ComputeEigensystemInputSpec(StdOutCommandLineInputSpec):
    ...
    inputmodel = traits.Enum(
        ...
        "multitensor",
        argstr="-inputmodel %s",
        desc='Specifies the model that the input data contains parameters for. Possible model types are: "dt" (diffusion-tensor data) and "multitensor"',
    )
