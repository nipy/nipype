    ...
    def _create_gradient_matrix(self, bvecs_file, bvals_file):
        ...
        bvals = [
            val for val in re.split('\s+',
                                    open(bvals_file).readline().strip())
        ]
...
class ODFReconInputSpec(CommandLineInputSpec):
    ...
    n_output_directions = traits.Int(
        desc='Number of output directions',
        argstr='%s',
        mandatory=True,
        position=3)
...
class ODFRecon(CommandLine):
    ...
    def _list_outputs(self):
        ...
        outputs['max'] = os.path.abspath(
            fname_presuffix(
                "", prefix=out_prefix, suffix='_max.' + output_type))
        outputs['ODF'] = os.path.abspath(
            fname_presuffix(
                "", prefix=out_prefix, suffix='_odf.' + output_type))
        if isdefined(self.inputs.output_entropy):
            outputs['entropy'] = os.path.abspath(
                fname_presuffix(
                    "", prefix=out_prefix, suffix='_entropy.' + output_type))
...
class ODFTrackerInputSpec(CommandLineInputSpec):
    ...
    invert_x = traits.Bool(
        argstr='-ix', desc='invert x component of the vector')
    invert_y = traits.Bool(
        argstr='-iy', desc='invert y component of the vector')
    invert_z = traits.Bool(
        argstr='-iz', desc='invert z component of the vector')
    swap_xy = traits.Bool(
        argstr='-sxy', desc='swap x and y vectors while tracking')
    swap_yz = traits.Bool(
        argstr='-syz', desc='swap y and z vectors while tracking')
    swap_zx = traits.Bool(
        argstr='-szx', desc='swap x and z vectors while tracking')
