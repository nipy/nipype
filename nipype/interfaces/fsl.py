...
class FilmGLS(FSLCommand):
    ...
    def _parse_inputs(self):
        ...
        if self.inputs.designfile:
            self.inputs.designfile = list_to_filename(self.inputs.designfile)
            allargs.append(self.inputs.designfile)
    ...
    def run(self, infile=None, designfile=None, thresh=None, **inputs):
        ...
        if os.access(os.path.join(cwd, self.inputs.rn), os.F_OK):
...
class Probtrackx(FSLCommand):
    ...
    def run(self, cwd=None, basename=None,binaryMask=None,seedFile=None,noseTest=False,**inputs):
        """Execute the command.
        >>> from nipype.interfaces import fsl
...
class Vecreg(FSLCommand):
    ...
    def run(self, infile=None, outfile=None, refVolName=None, cwd=None,**inputs):
        """Execute the command.
        >>> from nipype.interfaces import fsl
...
class Proj_thresh(FSLCommand):
    ...
    def run(self, cwd=None, volumes=None,threshold=None,**inputs):
        """Execute the command.
        >>> from nipype.interfaces import fsl
        >>> pThresh = fsl.Proj_thresh(volumes=['seeds_to_M1','seeds_to_M2'],threshold=3)
        >>> pThresh.cmdline
...
class Find_the_biggest(FSLCommand):
    ...
    def run(self, infile=None, outfile=None, cwd=None, **inputs):
        """Execute the command.
        >>> from nipype.interfaces import fsl
