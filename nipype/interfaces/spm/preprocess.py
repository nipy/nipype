...
class DARTELExistingTemplatesInputSpec(SPMCommandInputSpec):
    image_files = traits.List(traits.List(ImageFileSPM(exists=True)),
                              desc="A list of files to be segmented",
                              field='warp1.images', copyfile=False,
                              mandatory=True)
...
class DARTELExistingTemplatesOutputSpec(TraitedSpec):
    dartel_flow_fields = traits.List(File(exists=True),
                                     desc='DARTEL flow fields')


class DARTELExistingTemplates(SPMCommand):
    """Use spm DARTEL and existing templates to create flow fields
    ...
    --------
    >>> import nipype.interfaces.spm as spm
    >>> dartel = spm.DARTELExistingTemplates()
    >>> dartel.inputs.image_files = [['rc1s1.nii','rc1s2.nii'],['rc2s1.nii', 'rc2s2.nii']]
    >>> it1 = (3, (1, 0.5, 1e-6), 1, 'template_1.nii')
    >>> it2 = (3, (0.5, 0.25, 1e-6), 2, 'template_2.nii')
    >>> it3 = (3, (0.25, 0.125, 1e-6), 16, 'template_3.nii')
    >>> dartel.inputs.iteration_parameters = [it1, it2, it3]
    >>> dartel.inputs.optimization_parameters = (0.01, 3, 3)
    >>> dartel.run() # doctest: +SKIP

    """

    input_spec = DARTELExistingTemplatesInputSpec
    output_spec = DARTELExistingTemplatesOutputSpec
    _jobtype = 'tools'
    _jobname = 'dartel'
    ...
    def _format_arg(self, opt, spec, val):
        ...
        if opt in ['image_files']:
            ...
            return super(DARTELExistingTemplates, self)._format_arg(opt, spec, val)
    ...
    def _list_outputs(self):
        ...
        for filename in self.inputs.image_files[0]:
            pth, base, ext = split_filename(filename)
            outputs['dartel_flow_fields'].append(os.path.realpath('u_%s%s' %
                                                                  (base, ext)))
        return outputs
