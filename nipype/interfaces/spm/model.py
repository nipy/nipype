...
class GetTotalsModelInputSpec(SPMCommandInputSpec):
    in_image = File(exists=True, desc='mwimage to analyse', copyfile=False, mandatory=True)
    in_mask = File(exists=True, desc='mask to use', copyfile=False, mandatory=True)

class GetTotalsModelOutputSpec(TraitedSpec):
...
class GetTotals(SPMCommand):
    ...
    --------
    >>> gett = GetTotals()
    >>> gett.inputs.in_image = 'mwrc1subject1.nii'
    ...
    def _make_matlab_command(self, _):
        ...
        script += "% based on Ged Ridgway's get_totals.m - http://www0.cs.ucl.ac.uk/staff/g.ridgway/vbm/get_totals.m\n"
    ...
    def aggregate_outputs(self, runtime=None):
        ...
        for line in runtime.stdout.split('\n'):
            if line.startswith("totals = "):
                setattr(outputs, 'total_volume', float(line[len("totals = "):].strip()))
        return outputs
