...
class AutoTcorrelateInputSpec(AFNICommandInputSpec):
    ...
    out_file = File(name_template='%s_similarity_matrix.1D',
...
class AutomaskInputSpec(AFNICommandInputSpec):
    ...
    clfrac = traits.Float(desc='sets the clip level fraction (must be '\
                               '0.1-0.9). A small value will tend to make '\
                               'the mask larger [default = 0.5].',
                          argstr="-clfrac %s")
...
class BlurInMaskInputSpec(AFNICommandInputSpec):
    in_file = File(
        ...
        exists=True,
        copyfile=False)
    out_file = File(name_template='%s_blur', desc='output to the file',
                    argstr='-prefix %s', name_source='in_file', position=-1)
    mask = File(
...
class BlurToFWHMInputSpec(AFNICommandInputSpec):
    in_file = File(desc='The dataset that will be smoothed',
                   argstr='-input %s', mandatory=True, exists=True)

    automask = traits.Bool(desc='Create an automask from the input dataset.',
                           argstr='-automask')
    fwhm = traits.Float(desc='Blur until the 3D FWHM reaches this value (in mm)',
                        argstr='-FWHM %f')
    fwhmxy = traits.Float(desc='Blur until the 2D (x,y)-plane FWHM reaches '\
                               'this value (in mm)',
                          argstr='-FWHMxy %f')
    blurmaster = File(desc='The dataset whose smoothness controls the process.',
                      argstr='-blurmaster %s', exists=True)
    mask = File(desc='Mask dataset, if desired. Voxels NOT in mask will be '\
                     'set to zero in output.', argstr='-blurmaster %s',
                     exists=True)
...
class CalcInputSpec(AFNICommandInputSpec):
    ...
    expr = Str(desc='expr', argstr='-expr "%s"', position=3,
               mandatory=True)
...
class DegreeCentralityInputSpec(CentralityInputSpec):
    ...
    oned_file = Str(desc='output filepath to text dump of correlation matrix',
                argstr='-out1D %s')
...
class FimInputSpec(AFNICommandInputSpec):
    ...
    out = Str(desc='Flag to output the specified parameter',
...
class MaskToolInputSpec(AFNICommandInputSpec):
    ...
    dilate_inputs = Str(desc='Use this option to dilate and/or erode '+
                             'datasets as they are read. ex. ' +
                             '\'5 -5\' to dilate and erode 5 times',
                        argstr='-dilate_inputs %s')

    dilate_results = Str(desc='dilate and/or erode combined mask at ' +
                              'the given levels.',
    ...
    fill_dirs = Str(desc='fill holes only in the given directions. ' +
...
class RefitInputSpec(CommandLineInputSpec):
    ...
    xorigin = Str(desc='x distance for edge voxel offset',
                  argstr='-xorigin %s')

    yorigin = Str(desc='y distance for edge voxel offset',
                  argstr='-yorigin %s')
    zorigin = Str(desc='z distance for edge voxel offset',
                  argstr='-zorigin %s')
...
class ResampleInputSpec(AFNICommandInputSpec):
    ...
    orientation = Str(desc='new orientation code',
                      argstr='-orient %s')

    resample_mode = traits.Enum('NN', 'Li', 'Cu', 'Bk',
                                argstr='-rmode %s',
                                desc="resampling method from set {'NN', "\
                                     "'Li', 'Cu', 'Bk'}.  These are for "\
...
class RetroicorInputSpec(AFNICommandInputSpec):
    ...
    resp = File(desc='1D respiratory waveform data for correction',
                ...
                position=-3,
                exists=True)
    threshold = traits.Int(desc='Threshold for detection of R-wave peaks in '\
                                'input (Make sure it is above the background '\
                                'noise level, Try 3/4 or 4/5 times range '\
                                'plus minimum)',
                           argstr='-threshold %d',
                           position=-4)
...
class SegInputSpec(CommandLineInputSpec):
    ...
    classes = Str(desc='CLASS_STRING is a semicolon delimited string of class labels',
                  argstr='-classes %s')

    bmrf = traits.Float(desc='Weighting factor controlling spatial homogeneity of the classifications',
                        argstr='-bmrf %f')

    bias_classes = Str(desc='A semicolon delimited string of classes that '\
                            'contribute to the estimation of the bias field',
                       argstr='-bias_classes %s')

    prefix = Str(desc='the prefix for the output folder containing all output volumes',
                 argstr='-prefix %s')

    mixfrac = Str(desc='MIXFRAC sets up the volume-wide (within mask) tissue '\
                       'fractions while initializing the segmentation (see '\
...
class TCatInputSpec(AFNICommandInputSpec):
    ...
    rlt = Str(desc='options', argstr='-rlt%s', position=1)
...
class TShiftInputSpec(AFNICommandInputSpec):
    ...
    tr = Str(desc='manually set the TR. You can attach suffix "s" for seconds'\
    ...
    interp = traits.Enum(('Fourier', 'linear', 'cubic', 'quintic', 'heptic'),
                         desc='different interpolation methods (see 3dTShift '\
                         'for details) default = Fourier', argstr='-%s')

    tpattern = Str(desc='use specified slice time pattern rather than one in '\
                        'header',
                   argstr='-tpattern %s')
    ...
    rltplus = traits.Bool(desc='Before shifting, remove the mean and linear '\
                               'trend and later put back the mean',
                          argstr="-rlt+")
...
class TStatInputSpec(AFNICommandInputSpec):
    ...
    mask = File(desc='mask file',
                argstr='-mask %s',
                exists=True)
    options = Str(desc='selected statistical output',
                  argstr='%s')
...
class To3DInputSpec(AFNICommandInputSpec):
    ...
    funcparams = Str(desc='parameters for functional data',
...
class ZCutUpInputSpec(AFNICommandInputSpec):
    ...
    out_file = File(name_template="%s_zcupup", desc='output image file name',
                    argstr='-prefix %s', name_source="in_file")
    keep = Str(desc='slice range to keep in output',
               argstr='-keep %s')
