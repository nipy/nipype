...
class SVMTrainInputSpec(AFNICommandInputSpec):
    # training options
    ttype = traits.Str(
        desc='tname: classification or regression',
        argstr='-type %s',
        mandatory=True)
    in_file = File(
    ...
    nomodelmask = traits.Bool(
        desc='Flag to enable the omission of a mask file',
        argstr='-nomodelmask')
    trainlabels = File(
        desc=
...
class SVMTestInputSpec(AFNICommandInputSpec):
    ...
    testlabels = File(
        desc=
        '*true* class category .1D labels for the test dataset. It is used to calculate the prediction accuracy performance',
        exists=True,
        argstr='-testlabels %s')
    classout = traits.Bool(
        desc=
        'Flag to specify that pname files should be integer-valued, corresponding to class category decisions.',
        argstr='-classout')
