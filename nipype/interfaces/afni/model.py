...
class RemlfitInputSpec(AFNICommandInputSpec):
    ...
    nobout = traits.Bool(
        ...
        argstr='-nobout')
    gltsym = traits.List(
        traits.Either(traits.Tuple(File(exists=True), Str()),
    ...
    glt_file = File(
        ...
        argstr='-Rglt %s')
    fitts_file = File(
        desc='ouput dataset for REML fitted model',
        argstr='-Rfitts %s')
    errts_file = File(
...
class RemlfitOutputSpec(AFNICommandOutputSpec):
    ...
    obeta = File(
        desc='dataset for beta weights from the OLSQ estimation (if '
             'generated)')
    obuck = File(
    ...
    oerrts = File(
        desc='dataset for OLSQ residuals = data - fitted model (if '
             'generated')
