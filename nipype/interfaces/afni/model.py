...
""",
    ...
    groups = traits.List(
        Str(minlen=1),
        desc="""\
Name of 1 or 2 groups. This option must be used when comparing two groups.
Default is one group named 'G1'. The labels here are used to name the sub-bricks
in the output. When there are two groups, the 1st and 2nd labels here are associated
with the 1st and 2nd datasets specified respectively through option -set, and their
group difference is the second group minus the first one, similar to 3dttest but
...
0: With this format the zero value at a voxel of each subject
   will be interpreted as missing data.
File1 [File2]: Information about missing data is specified
...
class Mema(AFNICommand):
    ...
    .. testsetup::
        >>> create_testfiles(*[
        ...     'pb05.%s.%s.nii' % (n, c)
        ...     for n in ('Jane', 'John', 'Lisa', 'Amy', 'Josh', 'Mark')
        ...     for c in ('betas', 'tvals')])
        >>> create_testfiles('pb05.Jane.Regression+tlrc')
        >>> create_testfiles('covariates.txt')
    ...
    ... ])]
    >>> mema.cmdline
    "3dMEMA -no_model_outliers -no_residual_Z -set Placebo Jane \
pb05.Jane.Regression+tlrc'[face#0_Beta]' pb05.Jane.Regression+tlrc'[face#0_Tstat]' -verb 1"

    >>> mema.inputs.missing_data = 0
    >>> mema.cmdline
    "3dMEMA -missing_data 0 -no_model_outliers -no_residual_Z -set Placebo \
Jane pb05.Jane.Regression+tlrc'[face#0_Beta]' pb05.Jane.Regression+tlrc'[face#0_Tstat]' -verb 1"
    ...
    ...     ('Treatment', [
    ...         ('Amy', 'pb05.Amy.betas.nii', 'pb05.Amy.tvals.nii'),
    ...         ('Josh', 'pb05.Josh.betas.nii', 'pb05.Josh.tvals.nii'),
    ...         ('Mark', 'pb05.Mark.betas.nii', 'pb05.Mark.tvals.nii')])]
    >>> mema.cmdline # doctest: +IGNORE_EXCEPTION_DETAIL
    Traceback (most recent call last):
    ValueError:
    ...

    >>> mema.inputs.groups = ['Placebo', 'Treatment']
    >>> mema.cmdline
    '3dMEMA -groups Placebo Treatment \
-missing_data 0 -no_model_outliers -no_residual_Z -set Placebo \
Jane pb05.Jane.betas.nii pb05.Jane.tvals.nii \
John pb05.John.betas.nii pb05.John.tvals.nii \
Lisa pb05.Lisa.betas.nii pb05.Lisa.tvals.nii -set Treatment \
Amy pb05.Amy.betas.nii pb05.Amy.tvals.nii \
Josh pb05.Josh.betas.nii pb05.Josh.tvals.nii \
Mark pb05.Mark.betas.nii pb05.Mark.tvals.nii -verb 1'
    ...
    ...      '[face#0_Beta]', '[face#0_Tstat]')
    ... ])]
    >>> mema.inputs.nonzeros = 18
    >>> mema.cmdline
    "3dMEMA -n_nonzero 18.000000 -no_model_outliers -no_residual_Z \
-set Placebo Jane pb05.Jane.Regression+tlrc'[face#0_Beta]' \
pb05.Jane.Regression+tlrc'[face#0_Tstat]' \
-verb 1"

    >>> mema.inputs.hk_test = True
    >>> mema.cmdline
    "3dMEMA -HKtest -n_nonzero 18.000000 -no_model_outliers \
-no_residual_Z -set Placebo Jane pb05.Jane.Regression+tlrc'[face#0_Beta]' \
pb05.Jane.Regression+tlrc'[face#0_Tstat]' \
-verb 1"

    >>> mema.inputs.outliers = True
    >>> mema.cmdline
    "3dMEMA -HKtest -n_nonzero 18.000000 -model_outliers \
-no_residual_Z -set Placebo Jane pb05.Jane.Regression+tlrc'[face#0_Beta]' \
pb05.Jane.Regression+tlrc'[face#0_Tstat]' \
-verb 1"

    >>> mema.inputs.residualZ = True
    >>> mema.cmdline
    "3dMEMA -HKtest -n_nonzero 18.000000 -model_outliers \
-residual_Z -set Placebo Jane pb05.Jane.Regression+tlrc'[face#0_Beta]' \
pb05.Jane.Regression+tlrc'[face#0_Tstat]' \
-verb 1"

    >>> mema.inputs.equal_variance = False
    >>> mema.cmdline
    "3dMEMA -unequal_variance -HKtest -n_nonzero 18.000000 -model_outliers \
-residual_Z -set Placebo Jane pb05.Jane.Regression+tlrc'[face#0_Beta]' \
pb05.Jane.Regression+tlrc'[face#0_Tstat]' \
-verb 1"
    ...
    ...      '[face#0_Beta]', '[face#0_Tstat]')
    ... ])]
    >>> mema.inputs.covariates = 'covariates.txt'
    >>> mema.cmdline
    "3dMEMA -covariates covariates.txt \
-no_model_outliers -no_residual_Z -set Placebo \
Jane pb05.Jane.Regression+tlrc'[face#0_Beta]' \
pb05.Jane.Regression+tlrc'[face#0_Tstat]' \
-verb 1"

    >>> mema.inputs.covariates_center = 'age = 25 13 weight = 100 150'
    >>> mema.cmdline
    "3dMEMA -covariates covariates.txt -covariates_center age = 25 13 \
weight = 100 150 -no_model_outliers -no_residual_Z -set Placebo \
Jane pb05.Jane.Regression+tlrc'[face#0_Beta]' \
pb05.Jane.Regression+tlrc'[face#0_Tstat]' \
-verb 1"
    ...
    >>> mema.cmdline
    "3dMEMA -covariates covariates.txt -covariates_center age = 25 13 \
weight = 100 150 -covariates_model center=different slope=same \
-no_model_outliers -no_residual_Z -set Placebo \
Jane pb05.Jane.Regression+tlrc'[face#0_Beta]' \
pb05.Jane.Regression+tlrc'[face#0_Tstat]' \
-verb 1"
    ...
    "3dMEMA -covariates covariates.txt -covariates_center age = 25 13 \
weight = 100 150 -covariates_model center=different slope=same \
-prefix Results.BRIK -no_model_outliers -no_residual_Z -set Placebo \
Jane pb05.Jane.Regression+tlrc'[face#0_Beta]' \
pb05.Jane.Regression+tlrc'[face#0_Tstat]' \
-verb 1"
    ...
    def __init__(self, *args, **kwargs):
        self._n_sets = None
        return super(Mema, self).__init__(*args, **kwargs)
    ...
    def _parse_inputs(self, skip=None):
        self._n_sets = len(self.inputs.sets)
        if self._n_sets == 2 and not isdefined(self.inputs.groups):
            raise ValueError(
                'When two of ``-set`` are given, the option ``-groups`` must be defined.')
        elif isdefined(self.inputs.groups) and len(self.inputs.groups) != self._n_sets:
            raise ValueError(
                'Number of group names (``-groups``) and sets (``-set``) must match.')

        if (isdefined(self.inputs.missing_data) and isinstance(self.inputs.missing_data, list) and
                len(self.inputs.missing_data) != self._n_sets):
            raise ValueError(
                'Number of files given in ``-missing_data`` must match the number of sets.')
