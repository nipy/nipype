...
class FitAslInputSpec(NIFTYFITCommandInputSpec):
    ...
    m0mape = File(exists=True, desc='Filename of the estimated input M0 map error.',
                  argstr='-m0mape %s')
    IRvolume = File(exists=True, desc='Filename of a [1,2,5]s Inversion Recovery volume ' +
                                      '(T1/M0 fitting carried out internally).',
                    argstr='-IRvolume %s')
    ...
    L = traits.Float(desc='Single plasma/tissue partition coefficient [0.9ml/g].',
                     argstr='-L %f')
    eff = traits.Float(
        desc='Labelling efficiency [0.99 (pasl), 0.85 (pcasl)], ensure any background suppression pulses '
             + ' are included in -eff',
        argstr='-eff %f')
    ...
    LDD = traits.Float(desc='Labelling Duration [1800ms].',
                       argstr='-LDD %f')
    dPLD = traits.Float(desc='Difference in labelling delay per slice [0.0 ms/slice',
                        argstr='-dPLD %f')
    ...
    Tinv2 = traits.Float(desc='Inversion time [2000ms].',
                         argstr='-Tinv2 %f')
    dTinv2 = traits.Float(desc='Difference in inversion time per slice [0ms/slice]',
                          argstr='-dTinv2 %f')
    ...
    seg = File(exists=True,
               desc='Filename of the 4D segmentation (in ASL space) for L/T1 estimation and PV correction {WM,GM,CSF}.',
               argstr='-seg %s')
