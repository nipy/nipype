...
(e.g. 2.5)) [off].'
    ...
    t_inv1 = traits.Float(
        desc='Saturation pulse time [800ms].', argstr='-Tinv1 %f')
...
[0.25].'
    ...
    mulgm = traits.Bool(
        desc='Multiply CBF by segmentation [Off].', argstr='-sig')
    desc = 'Set PV threshold for switching off LSQR [O.05].'
    pv_threshold = traits.Bool(desc=desc, argstr='-pvthreshold')
    segstyle = traits.Bool(
        desc='Set CBF as [gm,wm] not [wm,gm].', argstr='-segstyle')
