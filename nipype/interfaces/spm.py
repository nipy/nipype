...
class Coregister(SpmMatlabCommandLine):
    ...
    def aggregate_outputs(self):
        ...
        if self.inputs.jobtype == "estimate":
            ...
            for f in filelist:
                c_file = glob(fname_presuffix(f,prefix='r',suffix=self.inputs.source[0][-4:],use_ext=False))
                assert len(c_file) == 1, 'No coregistered file generated by SPM Coregister'
                outputs.coregistered_files.append(c_file[0])
            outputs.coregistered_source = outputs.coregistered_files
        else:
            c_source = glob(fname_presuffix(self.inputs.source,prefix='r',suffix=self.inputs.source[0][-4:],use_ext=False))
            ...
            if self.inputs.apply_to_files:
                ...
                for f in filelist:
                    c_file = glob(fname_presuffix(f,prefix='r',suffix=self.inputs.apply_to_files[0][-4:],use_ext=False))
                    assert len(c_file) == 1, 'No coregistered file generated by SPM Coregister'
                    outputs.coregistered_files.append(c_file[0])
        return outputs
