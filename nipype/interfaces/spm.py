...
class SpmMatlabCommandLine(MatlabCommandLine):
    ...
    def generate_job(self,prefix,contents):
    ...
    def make_matlab_command(self, jobtype, jobname, contents, mfile=True,cwd='.'):
        ...
        if mfile:
            if jobname == 'smooth' or jobname == 'preproc':
                mscript += self.generate_job('jobs{1}.%s{1}.%s' % (jobtype,jobname) ,contents[0])
            else:
        ...
        return self.gen_matlab_command(mscript,cwd=cwd,script_name='pyscript_%s'%jobname)
...
class Realign(SpmMatlabCommandLine):
    ...
    def _compile_command(self,mfile=True):
        ...
        if self.inputs.write:
            ...
            jobtype = 'estimate'
        self.cmdline =self.make_matlab_command('spatial',
                                               'realign',
...
class Coregister(SpmMatlabCommandLine):
    ...
    def _parseinputs(self):
        ...
        for opt in inputs:
            if opt is 'target':
                einputs['ref'] = inputs[opt]
                continue
            if opt is 'source':
                einputs['source'] = inputs[opt]
                continue
    ...
    def _aggregate_outputs(self):
        ...
        outputs.coregistered_source = c_source
...
class Normalize(SpmMatlabCommandLine):
    ...
    def _parseinputs(self):
        ...
        for opt in inputs:
            if opt is 'template':
                einputs['eoptions'].update({'template': scans_for_fname(inputs[opt])})
                continue
            if opt is 'source':
                einputs['subj'].update({'source': scans_for_fname(inputs[opt])})
                continue
            if opt is 'apply_to_files':
                if type(inputs[opt]) == type([]):
                    sess_scans = scans_for_fnames(inputs[opt])
    ...
    def _aggregate_outputs(self):
        ...
        assert len(n_source) == 1, 'No normalized source file generated by SPM Normalize'
        ...
        if self.inputs.apply_to_files is not None:
            ...
            for f in filelist:
                ...
                assert len(n_file) == 1, 'No coregistered file generated by SPM Normalize'
...
class Segment(SpmMatlabCommandLine):
    ...
    tissue classes. Segment,  bias  correct  and spatially normalise -
    all in the same  model.  This  function can be used for bias
    correcting, spatially normalising or segmenting your data.
...
class Level1Design(SpmMatlabCommandLine):
    ...
    def _compile_command(self,mfile=True):
        ...
        """
        self.cmdline =self.make_matlab_command('stats',
                                               'fmri_spec',
                                               [self._parseinputs()],
                                               mfile,
                                               self.inputs.get('cwd','.'))
...
class EstimateModel(SpmMatlabCommandLine):
    ...
    def _aggregate_outputs(self):
        ...
        assert len(rpv) == 1, 'No spm mat files generated by SPM Estimate'
