...
class Realign(CommandLine):
    ...
    def _aggregate_outputs(self):
        ...
        if type(self.inputs.infile) == type([]):
            ...
            filelist = [self.inputs.infile]
        for f in filelist:
            r_files = glob(fname_presuffix(f,prefix='r',suffix='.nii',use_ext=False))
            assert len(r_files) >= 1, 'No realigned files generated by SPM Realign'
            outputs.realigned_files.append(r_files[0])
