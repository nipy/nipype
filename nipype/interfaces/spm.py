            ...
            for key,value in contents.items():
                if type(value) == type({}):
                    if value == {} or value is None:
                        ...
                        newdict.update({key:self.reformat_dict_for_savemat(value)})
                else:
...
class Realign(SpmMatlabCommandLine):
    ...
    def _aggregate_outputs(self):
        ...
        outputs = Bunch(realigned_files=None,
                        ...
                        mean_image=None)
        outputs.mean_image = glob(fname_presuffix(self.inputs.infile,prefix='mean'))[0]
...
class Level1Design(SpmMatlabCommandLine):
    ...
    def _compile_command(self,mfile=True):
        ...
        """
        self.cmdline,mscript =self.make_matlab_command('stats',
                                                       'fmri_spec',
                                                       [self._parseinputs()],
                                                       mfile,
                                                       self.inputs.get('cwd','.'))
