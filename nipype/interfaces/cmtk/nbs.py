...
class NetworkBrainStatistic(BaseInterface):
    ...
    def _run_interface(self, runtime):
        ...
        path = op.join(op.curdir,'NBS_Result_' + '_' + edge_key + '-thresh-' + str(THRESH) + '-k-' + str(K) + '-tail-' + TAIL + '.pck')
        iflogger.info(path)
        nx.write_gpickle(nbsgraph, path)
        iflogger.info('Saving output NBS edge network as {out}'.format(out=path))

        pval_path = op.join(op.curdir,'NBS_P_vals_' + '_' + edge_key + '-thresh-' + str(THRESH) + '-k-' + str(K) + '-tail-' + TAIL + '.pck')
    ...
    def _list_outputs(self):
        ...
        if self.inputs.output_as_cff:
            outputs['connectome_file'] = output_cff_name
        return outputs
