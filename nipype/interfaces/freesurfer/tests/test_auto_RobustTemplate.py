...
def test_RobustTemplate_inputs():
    input_map = dict(
        ...
        auto_detect_sensitivity=dict(
            argstr="--satit", mandatory=True, xor=["outlier_sensitivity"]
        ),
        average_metric=dict(argstr="--average %d"),
        environ=dict(nohash=True, usedefault=True),
        fixed_timepoint=dict(argstr="--fixtp"),
        in_files=dict(argstr="--mov %s", mandatory=True),
        ...
        num_threads=dict(),
        out_file=dict(
            argstr="--template %s", extensions=None, mandatory=True, usedefault=True
        ),
        outlier_sensitivity=dict(
            argstr="--sat %.4f", mandatory=True, xor=["auto_detect_sensitivity"]
        ),
