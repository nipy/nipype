...
from nipype.interfaces.base import InputMultiPath, TraitedSpec, traits, isdefined, File, Str
...
class CAT12SegmentInputSpec(SPMCommandInputSpec):
    ...
    affine_regularization = Str(default_value="mni",
                                field="opts.affreg", usedefault=True, desc=_help_affine_reg)

    _help_bias_acc = "Strength of the SPM inhomogeneity (bias) correction that simultaneously controls the SPM " \
                     "biasreg, biasfwhm, samp (resolution), and tol (iteration) parameter."
    power_spm_inhomogeneity_correction = traits.Float(default_value=0.5, field='opts.biasacc',
                                                      usedefault=True,
                                                      desc=_help_bias_acc)
    # Extended options for CAT12 preprocessing
    _help_app = 'Affine registration and SPM preprocessing can fail in some subjects with deviating anatomy (e.g. ' \
    ...
    affine_preprocessing = traits.Int(1070, field="extopts.APP", desc=_help_app, usedefault=True)

    _help_initial_seg = 'In rare cases the Unified Segmentation can fail in highly abnormal brains, where e.g. the ' \
                        ...
                        'required Values: \nnone: 0;\nlight: 1;\nfull: 2;\ndefault: 1070.'
    initial_segmentation = traits.Int(0, field="extopts.spm_kamap", desc=_help_initial_seg, usedefault=True)

    _help_las = 'Additionally to WM-inhomogeneities, GM intensity can vary across different regions such as the motor' \
    ...
    local_adaptive_seg = traits.Float(0.5, field="extopts.LASstr", usedefault=True, desc=_help_las)

    _help_gcutstr = 'Method of initial skull-stripping before AMAP segmentation. The SPM approach works quite stable ' \
                    ...
                    '\n - GCUT approach: 0.50; \n - APRG approach: 2'
    skull_strip = traits.Float(2, field="extopts.gcutstr", desc=_help_gcutstr, usedefault=True)

    _help_wmhc = 'WARNING: Please note that the detection of WM hyperintensies is still under development and does ' \
    ...
    wm_hyper_intensity_correction = traits.Int(1, field="extopts.WMHC", desc=_help_wmhc, usedefault=True)

    _help_vox = 'The (isotropic) voxel sizes of any spatially normalised written images. A non-finite value will be ' \
    ...
    _help_resampling = 'Internal resampling for preprocessing.\n The default fixed image resolution offers a good ' \
                       ...
                       '\nBest native: [0.5 0.1]'
    internal_resampling_process = traits.Tuple(traits.Float(1), traits.Float(0.1), minlen=2, maxlen=2, usedefault=True,
                                               field="extopts.restypes.optimal", desc="help_resampling")
    _errors_help = 'Error handling.\nTry to catch preprocessing errors and continue with the next data set or ignore ' \
                   ...
                   '\nValues:\nnone: 0,\ndefault: 1,\ndetails: 2.'
    ignore_errors = traits.Int(1, field="extopts.ignoreErrors", desc=_errors_help, usedefault=True)

    # Writing options
    _help_surf = 'Surface and thickness estimation. \nUse projection-based thickness (PBT) (Dahnke et al. 2012) to' \
    ...
    jacobianwarped = traits.Bool(True, field="output.jacobianwarped", usedefault=True, desc=_help_jacobian)

    # Deformation Fields
    _help_warp = 'Deformation fields can be saved to disk, and used by the Deformations Utility and/or applied to ' \
                 ...
                 '\ninverse + forward: [1 1]'
    warps = traits.Tuple(traits.Int(1), traits.Int(0), minlen=2, maxlen=2,
                         field="output.warps", usedefault=True, desc=_help_warp)
...
class CAT12Segment(SPMCommand):
    ...
    def _list_outputs(self):
        ...
        outputs["report_files"] = outputs["report_files"] = [str(report) for report in Path(pth).glob("report/*")
                                                             if report.is_file()]
