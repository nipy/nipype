...
def test_Registration_inputs():
    ...
    convergence_threshold=dict(requires=[u'number_of_iterations'],
    usedefault=True,
    ),
    convergence_window_size=dict(requires=[u'convergence_threshold'],
    usedefault=True,
    ),
    ...
    xor=[u'initial_moving_transform_com'],
    ),
    initial_moving_transform_com=dict(argstr='%s',
    xor=[u'initial_moving_transform'],
    ),
    ...
    invert_initial_moving_transform=dict(requires=[u'initial_moving_transform'],
    xor=[u'initial_moving_transform_com'],
    ),
    ...
    moving_image_mask=dict(requires=[u'fixed_image_mask'],
    ),
    ...
    requires=[u'output_warped_image'],
    ),
    ...
    radius_or_number_of_bins=dict(requires=[u'metric_weight'],
    usedefault=True,
    ),
    restore_state=dict(argstr='--restore-state %s',
    ),
    sampling_percentage=dict(requires=[u'sampling_strategy'],
    ),
    sampling_percentage_item_trait=dict(),
    sampling_percentage_stage_trait=dict(),
    sampling_strategy=dict(requires=[u'metric_weight'],
    ),
    ...
    sigma_units=dict(requires=[u'smoothing_sigmas'],
    ),
