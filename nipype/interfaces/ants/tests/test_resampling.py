...
def create_wimt():
    ...
    wimt.inputs.transformation_series = [
        ...
        "dwi2anat_Warp.nii.gz",
        "dwi2anat_coreg_Affine.txt",
    ]
    return wimt
...
def test_WarpImageMultiTransform_invaffine_2(change_dir, create_wimt):
    ...
    assert (
        wimt.cmdline
        == "WarpImageMultiTransform 3 diffusion_weighted.nii diffusion_weighted_wimt.nii -R functional.nii func2anat_coreg_Affine.txt func2anat_InverseWarp.nii.gz dwi2anat_Warp.nii.gz -i dwi2anat_coreg_Affine.txt"
    )
...
def test_WarpTimeSeriesImageMultiTransform(change_dir, create_wtsimt):
    ...
    assert (
        wtsimt.cmdline
        == "WarpTimeSeriesImageMultiTransform 4 resting.nii resting_wtsimt.nii \
