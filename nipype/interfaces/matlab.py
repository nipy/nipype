...
from .base import (CommandLineInputSpec, InputMultiObject, isdefined,
                   CommandLine, File, Directory, Unicode, List, Bool)
...
class MatlabInputSpec(CommandLineInputSpec):
    ...
    script = Unicode(argstr='-r \"%s;exit\"', desc='m-code to run',
                     mandatory=True, position=-1)
    uses_mcr = Bool(desc='use MCR interface',
                    xor=['nodesktop', 'nosplash',
                                'single_comp_thread'],
                    nohash=True)
    nodesktop = Bool(default_value=True, argstr='-nodesktop',
                     desc='Switch off desktop mode on unix platforms',
                     nohash=True)
    nosplash = Bool(default_value=True, argstr='-nosplash',
                    desc='Switch of splash screen',
                    nohash=True)
    logfile = File(argstr='-logfile %s',
                   desc='Save matlab output to log')
    single_comp_thread = Bool(argstr="-singleCompThread",
                              desc="force single threaded operation",
                              nohash=True)
    # non-commandline options
    mfile = Bool(default_value=True, desc='Run m-code using m-file')
    script_file = File('pyscript.m',
                       desc='Name of file to write m-code to')
    paths = InputMultiObject(Directory(), desc='Paths to add to matlabpath')
    prescript = List(default_value=["ver,", "try,"],
                     desc='prescript to be added before code')
    postscript = List(default_value=["\n,catch ME,",
                                     "fprintf(2,'MATLAB code threw an exception:\\n');",
                                     "fprintf(2,'%s\\n',ME.message);",
                                     "if length(ME.stack) ~= 0, fprintf(2,'File:%s\\nName:%s\\nLine:%d\\n',ME.stack.file,ME.stack.name,ME.stack.line);, end;",
                                     "end;"], desc='script added after code')
