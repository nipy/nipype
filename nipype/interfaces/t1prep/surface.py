...
:class:`T1PrepCatSurf`
    Per-vertex surface data post-processing (heat-kernel smoothing) via the
    ``cat_surf`` Python API, exposed by T1Prep as ``t1prep.cat_surf``.
"""
...
# ---------------------------------------------------------------------------
# T1PrepCatSurf – cat_surf surface post-processing
# ---------------------------------------------------------------------------


class T1PrepCatSurfInputSpec(BaseInterfaceInputSpec):
    """Input specification for :class:`T1PrepCatSurf`."""

    in_surface = File(
        exists=True,
        mandatory=True,
        desc=(
            "Input central surface file in GIFTI (.gii), FreeSurfer, or "
            "BIC/MNI (.obj) format."
        ),
    )
    in_values = File(
        ...
        mandatory=True,
        desc=(
            "Per-vertex scalar data file to smooth "
            "(e.g. cortical thickness or surface area)."
        ),
    )
    fwhm = traits.Float(
        20.0,
        usedefault=True,
        desc=(
            "Full-width at half-maximum (mm) of the heat kernel applied to "
            "the per-vertex data (default 20 mm)."
        ),
    )
    out_file = File(
        desc=(
            "Output file path for the smoothed values.  If not specified, "
            "a name is generated automatically by prepending 's{fwhm}' to "
            "the input values filename."
        ),
    )


class T1PrepCatSurfOutputSpec(TraitedSpec):
    """Output specification for :class:`T1PrepCatSurf`."""

    out_file = File(exists=True, desc="Smoothed per-vertex values file.")


class T1PrepCatSurf(BaseInterface):
    """Nipype interface for ``t1prep.cat_surf`` surface post-processing.

    Applies heat-kernel smoothing to per-vertex surface data (e.g. cortical
    thickness maps produced by :class:`T1PrepSurfaceEstimation`) using the
    ``cat_surf`` Python API, which T1Prep re-exports as ``t1prep.cat_surf``.

    ``cat_surf`` provides Python bindings to the CAT-Surface C library and is
    the same back-end used internally by T1Prep's surface estimation pipeline.
    Key operations available in the API include:

    * ``cat_surf.smooth_heatkernel``   – heat-kernel smoothing (this interface)
    * ``cat_surf.get_area``            – per-vertex surface area
    * ``cat_surf.euler_characteristic``– topological integrity check
    * ``cat_surf.resample_to_sphere``  – resample to a standard sphere
    * ``cat_surf.vol_sanlm``           – SANLM denoising of volumes
    * ``cat_surf.vol_marching_cubes``  – surface extraction from a volume
    * ``cat_surf.vol2surf``            – project a volume onto a surface
    ...
    >>> smooth.inputs.out_file = 'lh.thickness_s20.sub-01_T1w'

    References
    ----------
    https://github.com/ChristianGaser/CAT-Surface
    https://pypi.org/project/cat-surf/
    """

    input_spec = T1PrepCatSurfInputSpec
    ...
    def _run_interface(self, runtime):
        ...
        cat_surf.write_values(self._gen_outfile(), smoothed)
        return runtime

    def _list_outputs(self):
        outputs = self._outputs().get()
        outputs["out_file"] = self._gen_outfile()
        return outputs

    def _gen_outfile(self):
        """Return the absolute path for the smoothed output values file."""
        if isdefined(self.inputs.out_file):
            return os.path.abspath(self.inputs.out_file)
        in_vals = self.inputs.in_values
        basename = os.path.basename(in_vals)
        fwhm_tag = f"s{int(round(self.inputs.fwhm))}"
        return os.path.abspath(f"{fwhm_tag}.{basename}")
