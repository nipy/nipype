...
class CoherenceAnalyzerInputSpec(BaseInterfaceInputSpec):
    ...
    in_file = File(
        desc=(
            "csv file with ROIs on the columns and "
    ...
    NFFT = traits.Range(
        ...
        usedefault=True,
        desc=(
            "This is the size of the window used for "
    ...
    n_overlap = traits.Range(
        ...
        usedefault=True,
        desc=(
            "The number of samples which overlap"
...
``file_name_{coherence,timedelay}``. Possible formats: .png,.svg,.pdf,.jpg,..."""
    ...
    figure_type = traits.Enum(
        ...
        usedefault=True,
        desc=(
            "The type of plot to generate, where "
...
class CoherenceAnalyzer(NitimeBaseInterface):
    ...
    def _run_interface(self, runtime):
        ...
        freq_idx = np.where(
            ...
            * (A.frequencies < self.inputs.frequency_range[1])
        )[0]
    ...
    def _make_output_figures(self):
        ...
        if self.inputs.figure_type == "matrix":
            ...
            fig_coh.savefig(
                fname_presuffix(self.inputs.output_figure_file, suffix="_coherence")
            )
            ...
            fig_dt.savefig(
                fname_presuffix(self.inputs.output_figure_file, suffix="_delay")
            )
        else:
