...
class ComputeMaskInputSpec(BaseInterfaceInputSpec):
    ...
    reference_volume = File(
        exists=True,
        desc=(
            "reference volume used to compute the mask. "
...
class ComputeMask(NipyBaseInterface):
    ...
    def _run_interface(self, runtime):
        ...
        for key in [
            k
            for k, _ in list(self.inputs.items())
...
class SpaceTimeRealignerInputSpec(BaseInterfaceInputSpec):
    in_file = InputMultiPath(
        File(exists=True), mandatory=True, min_ver="0.4.0.dev", desc="File to realign"
    )
    ...
    slice_info = traits.Either(
        ...
        desc=(
            "Single integer or length 2 sequence "
...
class TrimInputSpec(BaseInterfaceInputSpec):
    ...
    end_index = traits.Int(
        0, usedefault=True, desc="last volume indexed as in python (and 0 for last)"
    )
    ...
    def _list_outputs(self):
        ...
        if not isdefined(outputs["out_file"]):
            outputs["out_file"] = fname_presuffix(
                self.inputs.in_file, newpath=os.getcwd(), suffix=self.inputs.suffix
            )
