...
class MultiProcPlugin(DistributedPluginBase):
    ...
    def __init__(self, plugin_args=None):
        ...
        if self.n_gpus > self.n_gpus_visible:
            logger.info('Total number of GPUs (%d) requested exceeds '
                        'available number of GPUs (%d). Using all %d GPU(s).',
                        self.n_gpus, self.n_gpus_visible, self.n_gpus_visible)
    ...
    def set_gpu_slot_busy(self, slotno, jobid):
        # if a GPU has free slot, book it for a jobid,modify the queue and set
        # its slotno busy
        devno = None
        for dk in self.gpu_q.keys():
    ...
    def _task_finished_cb(self, jobid):
        ...
        # update queue status
        was_gpu_job = (
            hasattr(self.procs[jobid]._interface.inputs, 'use_cuda') or
            hasattr(self.procs[jobid]._interface.inputs, 'use_gpu'))
        if was_gpu_job:
            devid = self.set_gpu_slot_free(jobid)
            if devid is not None:
                logger.info('Device %d slot set free from jobid %d', devid,
                            jobid)
    ...
    def _prerun_check(self, graph):
        ...
        if np.any(np.array(tasks_gpu_th) > self.total_gpu_processors):
            ...
            if self.raise_insufficient:
                raise RuntimeError('Insufficient GPU resources available for '
                                   'job')
    ...
    def _check_resources(self, running_tasks):
        ...
        for _, jobid in running_tasks:
            is_gpu_job = (
                ...
                hasattr(self.procs[jobid]._interface.inputs, 'use_gpu'))
            if is_gpu_job:
                free_gpu_slots -= min(self.procs[jobid].n_procs,
                                      free_gpu_slots)
    ...
    def _send_procs_to_workers(self, updatehash=False, graph=None):
        ...
        free_memory_gb, free_processors, free_gpu_slots = \
            self._check_resources(self.pending_tasks)
        ...
        if self._stats != stats:
            ...
            logger.info(
                ...
                'memory (GB): %0.2f/%0.2f, Free processors: %d/%d, Free '
                'GPUs: %d/%d.%s',
        ...
        for jobid in jobids:
            ...
            if is_gpu_job:
                next_job_gpu_th = min(self.procs[jobid].n_procs,
                                      self.total_gpu_processors)
            else:
            ...
            elif (not is_gpu_job and next_job_th > free_processors or
                  next_job_gb > free_memory_gb):
                # If node does not fit, skip at this moment
                logger.debug('Cannot allocate job %d (%0.2fGB, %d threads, '
                             '%d slots).', jobid, next_job_gb, next_job_th,
                             next_job_gpu_th)
                continue
            ...
            if is_gpu_job:
                ...
                if devno is not None and slotno is not None:
                    ...
                    logger.info(
                        '[*** GPU ID: %d  Running: %s Job-ID: %d, Slot-ID:%d, '
                        'Queue State:%s', devno, self.procs[jobid]._id, jobid,
                        slotno, json.dumps(self.gpu_q))
    ...
    def _sort_jobs(self, jobids, scheduler='tsort'):
        if scheduler == 'mem_thread':
            return sorted(
                jobids,
                key=lambda item: (self.procs[item].mem_gb,
                                  self.procs[item].n_procs)
            )
        return jobids
