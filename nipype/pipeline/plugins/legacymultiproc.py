...
try:
    ...
    _nondaemon_context_mapper = {"spawn": NonDaemonSpawnContext()}
...
class LegacyMultiProcPlugin(DistributedPluginBase):
    ...
    def __init__(self, plugin_args=None):
        ...
        logger.debug(
            '[LegacyMultiProc] Starting in "%sdaemon" mode (n_procs=%d, '
            "mem_gb=%0.2f, cwd=%s)",
            "non" * int(non_daemon),
            self.processors,
            self.memory_gb,
    ...
    def _submit_job(self, node, updatehash=False):
        ...
        self._task_obj[self._taskid] = self.pool.apply_async(
            run_node, (node, updatehash, self._taskid), callback=self._async_callback
        )
