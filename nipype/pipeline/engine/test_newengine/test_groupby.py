...
@pytest.mark.parametrize("inputs_dict, expected_output", [ 
        ({"a": [3, 4, 5]},
         # output is 1dimensional, out[0] is related to a[0]=3
         # a[:][1] is 0-dimensional, i.e. one element 
         # there is no othe inputs states, so dictionary in a[:][1][0] is empty
         [({"a":3}, ({}, 9)), ({"a":4}, ({}, 16)), ({"a":5}, ({}, 25))]), 
...
@pytest.mark.parametrize("inputs_dict, expected_output", [
        ({"a":[3, 1, 8], "b":[0, 1, 2]}, 
         # output is 1dimensional, out[0] is for a[0]=3, etc.
         # since mapper was a.b out[0][1] has only one element, for b[0]=0 
...
@pytest.mark.parametrize("inputs_dict, expected_output", [
        ({"a":[3, 1], "b":[1, 2, 4]}, 
         # output is 1dimensional, out[0] is for a[0]=3, etc.,
         # the mapper was axb, so for every element a we have 4 elements (for 4 elements of b)
         # out[0][1] is 1dimensional and a[0][1][0] is for b[0]=1 
         [({"a":3}, [({"b":1}, 3), ({"b":2}, 6), ({"b":4}, 12)]), 
...
@pytest.mark.parametrize("inputs_dict1, inputs_dict2, expected_output", [
        ({"a": [3, 4, 5]}, {"b": [10, 20, 30]},
         # the default mapper is N1.axb, so after grouping with N1.a 
         # gives a similar structure as in test_single_groupbyind_4
         # out[0] is for N1.a[0]=3
         # out[0][1] is 1-dimens. and out[0][1][0] is for b[0]=10
...
def test_2nodes_groupbyind_1(inputs_dict1, inputs_dict2, expected_output):
    ...
    N2 =  Node(inputs=inputs_dict2, interface=fun4, groupByInd="N1.a")
...
@pytest.mark.parametrize("inputs_dict1, inputs_dict2, expected_output", [
        ({"a": [3, 4, 5]}, {"b": [10, 20, 30]},
         # the default mapper is N1.axb, so after grouping with b is 1-dimensional
         # out[0] is for b[0]=10
         # out[0][1] is 1-dimens. and out[0][1][0] is for N1.a[0]=3
         # the test has the same elements as test_2nodes_groupbyind_1 but grouped differently
...
@pytest.mark.parametrize("inputs_dict1, inputs_dict2, expected_output", [
        ({"a": [3, 4]}, {"b": [10, 20, 30]},
         # output is 1dimensional and out[0] is for N1.a[0]=3
         # output of N1 is a vector (1dim array), but mapper is A.b
         # so out[0][1] is also 1dimensional, 
         # out[0][1][0] is for A[0]=1 and b[0]=10, etc.
...
@pytest.mark.parametrize("inputs_dict1, inputs_dict2, expected_output", [
        ({"a": [3, 4]}, {"b": [10, 20, 30]},
         # output is 1dimensional and out[0] is for b[0]=10
         # Since mapper is A.b, out[0][1] is also 1dimensional
         # out[0][1] is for A[0]=1 
         # out[0][1][0] is for N1.a[0]=3, etc.
