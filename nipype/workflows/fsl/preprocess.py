...
def create_register_workflow(name='register'):
    """Create a workflow to register to a set of files to a target using flirt

    Parameters
    ----------

    name : name of workflow (default: register)

    Inputs::

        inputspec.reference_file : target file to register to
        inputspec.source_files : set of 3d files to register to reference
        inputspec.apply_to_files : set of files to register to reference using
           the registration derived from source_files. the number of source and
           apply_to_files must be the same.

    Outputs::

        outputspec.registered_files : registered files
        outputspec.reg_files : registration files
    ...
    """
    Set up a node to define all inputs required for the registration workflow
    """

    inputnode = pe.Node(interface=util.IdentityInterface(fields=['reference_file',
                                                                 'source_files',
                                                                 'apply_to_files']),
                            name='inputspec')
    outputnode = pe.Node(interface=util.IdentityInterface(fields=['registered_files',
                                                                  'reg_files']),
                         name='outputspec')
    ...
    flirt = pe.MapNode(fsl.FLIRT(dof=6),
                       iterfield=['in_file'],
                       name='calcreg')
    register.connect(inputnode, 'reference_file', flirt, 'reference_file')
    register.connect(inputnode, 'source_files', flirt, 'in_file')

    merge = pe.MapNode(fsl.Merge(dimension='t'),
                       iterfield=['in_files'],
                       name='merge')

    applyreg = pe.MapNode(fsl.ApplyXfm(apply_xfm=True),
                          iterfield=['in_file'],
                          name='applreg')

    register.connect(inputnode, 'apply_to_files', merge, 'in_files')
    register.connect(merge, 'merged_file', applyreg, 'in_file')
    register.connect(flirt, 'out_matrix_file', applyreg, 'in_matrix_file')
    register.connect(flirt, 'out_matrix_file', outputnode, 'reg_files')

    split = pe.MapNode(fsl.Split(dimension='t'),
                       iterfield=['in_files'],
                       name='merge')
    register.connect(applyreg, 'out_file', split, 'in_file')
    register.connect(split, 'out_files', outputnode, 'registered_files')
