...
def motion_correct(name='motion_correct'):
    ...
    >>> mc = motion_correct()
    >>> mc.inputs.inputnode.in_file = 'diffusion.nii'
    >>> mc.inputs.inputnode.in_bvec = 'diffusion.bvec'
    >>> mc.inputs.inputnode.ref_num = 0
    >>> mc.run() # doctest: +SKIP

    Inputs::

        inputnode.in_file - input dwi file
        inputnode.ref_num - index of the `b0` volume that should be taken as reference
    ...
    wf.connect([
        ...
        ,(inputnode,  pick_ref,   [(('ref_num', _checkset), 'index')])
...
###############################################################################
# Old workflows, mark deprecated
###############################################################################

def create_motion_correct_pipeline(name='create_motion_correct_pipeline'):
    """
    .. deprecated:: 0.9.2.
       Please use :func:`.motion_correct` instead

    """
    return motion_correct(name)
...
def create_dmri_preprocessing(name='dMRI_preprocessing', use_fieldmap=True, fieldmap_registration=False):
    ...
    .. warning:: IMPORTANT NOTICE: this workflow rotates the b-vectors, so please be advised \
    that not all the dicom converters ensure the consistency between the resulting \
...
def avg_b0(in_dwi, in_bval, out_file=None):
    """
    A function that averages *b0* volumes from a DWI dataset.
