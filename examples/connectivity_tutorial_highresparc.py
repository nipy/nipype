...
fs_dir = os.path.abspath('/software/freesurfer')
subjects_dir = os.path.abspath(op.join(op.curdir,'./subjects'))

"""
This needs to point to the fdt folder you can find after extracting
http://www.fmrib.ox.ac.uk/fslcourse/fsl_course_data2.tar.gz
"""
data_dir = os.path.abspath(op.join(op.curdir,'exdata/'))
...
roigen.inputs.LUT_file = './FreeSurferColorLUT_adapted.txt'
...
mapping.connect([(inputnode, bbregister,[("subject_id","subject_id")])])
...
mapping.connect([(inputnode, bbregister,[('dwi','source_file')])])
...
mapping.connect([(inputnode, ApplyVolTransform_Brain,[('dwi','source_file')])])
...
mapping.connect([(inputnode, ApplyVolTransform_WMParc,[('dwi','source_file')])])
...
mapping.connect([(inputnode, ApplyVolTransform_AparcAseg,[('dwi','source_file')])])
#mapping.connect([(mri_convert_AparcAseg, ApplyVolTransform_AparcAseg,[('out_file','target_file')])])
...
#mapping.connect([(ApplyVolTransform_AparcAseg, roigen,[("transformed_file","aparc_aseg_file")])])
