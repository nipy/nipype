...
"""
   g. Use :class:`nipype.interfaces.spm.Segment` to warp structural
   data to SPM's template. 
"""
segment = nw.NodeWrapper(interface=spm.Segment())

normalizefunc = nw.NodeWrapper(interface=spm.Normalize(),
                               name='Normfunc.spm')
normalizefunc.inputs.jobtype = 'write'

normalizestruct = nw.NodeWrapper(interface=spm.Normalize(),
                               name='Normstruct.spm')
normalizestruct.inputs.jobtype = 'write'
...
l1pipeline.connect([(datasource, realign,[('func','infile')]),
		  ...
		  (datasource, segment,[('struct', 'data')]),
                  (segment, normalizefunc, [('transformation_mat', 'parameter_file')]), 
                  (segment, normalizestruct, [('transformation_mat', 'parameter_file')]), 
		  (coregister, normalizefunc, [('coregistered_source','source'),
                                               ('coregistered_files','apply_to_files')]),
		  (segment, normalizestruct,[('modulated_input_image', 'apply_to_files')]),
		  (normalizefunc, smooth, [('normalized_files', 'infile')]),
                  ...
                  (normalizefunc,skullstrip,[('normalized_source','infile')]),
                  (realign,art,[('realignment_parameters','realignment_parameters')]),
                  (normalizefunc,art,[('normalized_files','realigned_files')]),
